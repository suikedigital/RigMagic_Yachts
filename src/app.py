...
def search_yachts(query: str = Query("", description="Free-form search query")):
    ...
    results = sorted(results, key=lambda x: int(x.get("yacht_id", 0)), reverse=True)[
        :10
    ]
    return results
    ...
    if req.possible_sails:
        ...
        try:
            for sail_type in req.possible_sails:
                ...
                logger.info(
                    f"[Orchestrator] POST /sails/possible/{yacht_id} payload: "
                    f"{sail_payload}"
                )
                resp = requests.post(
    ...
    if req.possible_ropes:
        ...
        try:
            for rope_type in req.possible_ropes:
                ...
                logger.info(
                    f"[Orchestrator] POST /ropes/possible/{yacht_id} payload: "
                    f"{rope_payload}"
                )
                resp = requests.post(
...
def add_yacht_to_user(user_id: str, yacht_id: int):
    ...
    if resp.status_code != 200:
        raise HTTPException(
            ...
            detail=f"Failed to add yacht to user: {resp.text}",
        )
    return resp.json()


def clone_yacht(
    yacht_id: int, user_id: int, name: str = None, spec: str = None, notes: str = None
):
    """
    ...
    """
    print(
        f"[clone_yacht] called with yacht_id={yacht_id}, user_id={user_id}, "
        f"name={name}, spec={spec}, notes={notes}",
        flush=True,
    )
    logger.info(
        f"[clone_yacht] called with yacht_id={yacht_id}, user_id={user_id}, "
    ...
    try:
        ...
        print(
            f"[clone_yacht] get_yacht returned: {yacht}", flush=True
        )
        if hasattr(yacht, "dict"):
            yacht_data = yacht.dict()
        else:
            yacht_data = dict(yacht)
        ...
        print(
            f"[clone_yacht] yacht_data for create_yacht: {yacht_data}", flush=True
        )
        ...
        if not new_yacht_id:
            ...
            raise HTTPException(
                status_code=500, detail="Could not determine new yacht_id"
            )
            ...
            for sail_type in possible_sails:
                sail_payload = {
                    ...
                    "sail_type": (
                        sail_type
                        if isinstance(sail_type, str)
                        else sail_type.get("sail_type") or sail_type.get("type")
                    ),
                    "config": (
                        sail_type.get("config") if isinstance(sail_type, dict) else None
                    ),
                ...
                logger.info(
                    f"[Orchestrator] POST /sails/possible/{new_yacht_id} payload: "
                    f"{sail_payload}"
                )
                requests.post(
                    f"{SAILS_API}/sails/possible/{new_yacht_id}",
                    json=sail_payload,
                    timeout=5,
                )
        if possible_ropes:
            for rope_type in possible_ropes:
                ...
                logger.info(
                    f"[Orchestrator] POST /ropes/possible/{new_yacht_id} payload: "
                    f"{rope_payload}"
                )
                requests.post(
                    f"{ROPES_API}/ropes/possible/{new_yacht_id}",
                    json=rope_payload,
                    timeout=5,
                )
        add_yacht_to_user(user_id, new_yacht_id)
        print(
            f"[clone_yacht] Successfully cloned yacht. new_yacht_id={new_yacht_id}",
            flush=True,
        )
        logger.info(
            f"[clone_yacht] Successfully cloned yacht. new_yacht_id={new_yacht_id}"
        ...
        raise HTTPException(
            status_code=e.status_code, detail=f"Yacht not found: {e.detail}"
        )
    except Exception as e:
...
def clone_yacht_endpoint(data: dict = Body(...)):
    ...
    try:
        result = clone_yacht(
            original_yacht_id, user_id, name=name, spec=spec, notes=notes
        )
